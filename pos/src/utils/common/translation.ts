// Create a separate file: translations.js
import { sinhalaBill } from "@/data";

// ---- Pre-compute Sinhala labels ONCE ----
const sinhalaHeadings = {
  BRANCH: "ශාඛාව",
  COUNTER: "කවුළුව",
  INVOICE_NO: "බිල්පත් අංකය",
  QUANTITY: "ප්‍රමාණය",
  MARKED_PRICE: "සඳහන් මිල",
  SELLING_PRICE: "අපේ මිල",
  UNIT_SUM_PRICE: "එකතුව",
  TOTAL_SUM_PRICE: "මුළු එකතුව",
  DISCOUNT: "ඔබේ ලාභය",
  DELIVERY: "ප්‍රවාහන ගාස්තු",
  INITIAL_PAYMENT_MODE: "ගෙවුම් මාධ්‍ය",
  PAYMENT_TYPE: "ගෙවුම් වර්ගය",
  PAID_AMOUNT: "ගෙවූ මුදල",
  PAYMENT_BALANCE: "ඉතිරි මුදල",
  STATUS: "ඇණවුමේ තත්ත්වය",
  DATE_TIME: "දිනය හා වේලාව",
};
...
const englishHeadings = {
  ...
  SELLING_PRICE: "Our Price",
  UNIT_SUM_PRICE: "Total",
  TOTAL_SUM_PRICE: "Total",
  DISCOUNT: "Discount Received",
  DELIVERY: "Delivery Charge",
  INITIAL_PAYMENT_MODE: "Initial Payment Mode",
...
export const getHeadings = () => {
  return sinhalaBill ? sinhalaHeadings : englishHeadings;
};

export const getPaymentModValue = (method) => {
  if (!sinhalaBill) return method;
...
export const getStatusValue = (status) => {
  ...
  const labels = {
    ...
    Shipped: "යවා ඇත",
    Cancelled: "අවලංගුයි",
    Returned: "ආපසු ලබා ඇත",
  };
  return labels[status] || status;
};

// unicode from the scratch
