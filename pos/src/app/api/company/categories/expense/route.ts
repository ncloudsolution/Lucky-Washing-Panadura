...
export const POST = auth(async function POST(req) {
  ...
  try {
    ...
    const currentCategories = business?.expenseCategories || [];
...
export const DELETE = auth(async function DELETE(req) {
  try {
    ...
    const categoryInUse = await prisma.income.findFirst({
      where: {
        category: category,
      },
      select: { id: true },
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "This Income Category is already used by Incomes and cannot be deleted",
        },
        { status: 409 }
      );
    }
    ...
    return NextResponse.json(
      {
        success: true,
        message: `Income Category ${category} deleted sucessfully`,
        data: null,
      },
      { status: 200 }
    );
